...
fun SittoStandCard(navController: NavController) {
    ...
    fun updatePatientWithTestResult(patient: Patient, timeTaken: Long?) {
        ...
        if (timeTaken != null) {
            timeTaken?.let {
                ...
                val timeString = "Sitting to Standing: ${seconds}s   ${milliseconds}ms"
                val updatedPatient = patient.copy(
                    stsTest = patient.stsTest + timeString
                )
                val patientRef = firestore.collection("Patient").document(patient.serialNo)
                patientRef.set(updatedPatient)
                                ...
                                collectionJob = mainScope.launch {
                                    ...
                                    testCommand(command).collect { (client, server) ->
                                        ...
                                        if (client == "1" && server == "1" && timerRunning.value) {
                                            ...
                                            collectionJob = null
                                            updatePatientWithTestResult(patient, timeTaken.value)
                                        }
                                    }
                                }
                            } else {
